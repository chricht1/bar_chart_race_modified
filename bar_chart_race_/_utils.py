...
def get_wide_data(fname):
    wide_fname = fname + '_wide.csv'
    wide_fpath = os.path.join('./data', wide_fname)
    if os.path.isfile(wide_fpath):
        return pd.read_csv(wide_fpath, index_col='window')
    
    data_fpath = os.path.join('./data',fname+'.csv')
    df = pd.read_csv(data_fpath, index_col='global_index')

    new_df = df.pivot(index='window', columns='pathway.name', values='-log10(p.adj)')

    new_df.to_csv(wide_fpath)
